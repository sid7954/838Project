...
def train(train_loader, model, criterion, optimizer, epoch, target_rates):
    ...
    male_mask[[23,25,27]] = 0.0
    female_mask[[24,26,28]] = 0.0        

    # Age bracket masks
    bracket_masks = np.ones((5,33), dtype='float')
    zeros_ind = {}
    zeros_ind[0] = []  #all layers active for unknown age
    zeros_ind[1] = [7,9,11,13]
    zeros_ind[2] = [15,17,19,21]
    zeros_ind[3] = [8,10,12,14]
    zeros_ind[4] = [16,18,20,22]
    for i in range(5):
        bracket_masks[zeros_ind[i]] = 0.0 
    ...
    for i, (input, target, gender, age_bracket) in enumerate(train_loader):
        # input = input.view((input.size(0), input.size(2), input.size(3), input.size(1)))
        #print(i)
       
        target = target.cuda(async=True)
        gender = gender.cuda(async=True).float()
        age_bracket = age_bracket.cuda(async=True).float()
        input = input.cuda()
        ...
        gender_bias_loss = torch.mean((1-gender)*torch.mean(torch.pow(act - male_mask_t,2), 1) 
            + gender*torch.mean(torch.pow(act-female_mask_t, 2), 1))

        age_bias_loss = 0
        for i in range(5):
            age_bias_loss += age_bracket[i]*torch.mean(torch.pow(act - bracket_masks[i],2),1)
        age_bias_loss = torch.mean(age_bias_loss, 1)

        gender_bias_loss *= 1
        age_bias_loss *= 1

        loss = loss_classify + gender_bias_loss + age_bias_loss
...
def validate(val_loader, model, criterion, epoch, target_rates):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, gender, age_bracket) in enumerate(val_loader):
            target = target.cuda(async=True)
            input = input.cuda()
            input_var = torch.autograd.Variable(input)
            target_var = torch.autograd.Variable(target)
