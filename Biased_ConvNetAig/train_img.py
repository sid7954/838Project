...
from dataset import FacesDataset_with_Gender as FacesDataset
...
def train(train_loader, model, criterion, optimizer, epoch, target_rates):
    ...
    male_mask = np.ones(33, dtype='float')
    female_mask = np.ones(33, dtype='float')
    for i in range(7,30):
        if i%2==0:
            male_mask[i] = 0.0
        else:
    ...
    male_mask_t = torch.Tensor(male_mask).cuda(async=True)
    female_mask_t = torch.Tensor(female_mask).cuda(async=True)
    ...
    for i, (input, target, gender) in enumerate(train_loader):
        ...
        gender = gender.cuda(async=True).float()
        input = input.cuda()
        ...
        #acts_tensor = torch.Tensor(activation_rates)

        #print(gender.size(), male_mask_t.size(), female_mask_t.size())

        act = torch.stack(activation_rates).view(len(activation_rates), -1).t()
        #print(act.size(), act.type(), male_mask_t.type(), gender.type())

        bias_loss = torch.mean((1-gender)*torch.mean(torch.pow(act - male_mask_t,2), 1) + gender*torch.mean(torch.pow(act-female_mask_t, 2), 1))
        #print(bias_loss)
        '''
        bias_loss = 0
        for k in range(len(activation_rates)):
            act = activation_rates[k]
            #print(act)
            for j in range(args.batch_size):
                bias_loss += (1-gender[j])*torch.mean(torch.pow(act[j]-male_mask_t[k],2)) + gender[j]*torch.mean(torch.pow(act[j]-female_mask_t[k],2))
        '''
        ...
        for j, act in enumerate(activation_rates):
            for i in range(33):
                bias_loss += (1-gender[i])*torch.mean(torch.pow(act-male_mask_t,2)) + gender[i]*torch.mean(torch.pow(act-female_mask_t,2))
#            print(j, act, male_mask, female_mask, bias_loss)

#        bias_loss = torch.mean(torch.pow(acts_tensor*male_mask_t, 2) + torch.pow(1-acts_tensor*female_mask_t, 2))
        '''

        bias_loss *= 1

        loss = loss_classify + bias_loss
        ...
        losses_b.update(bias_loss.item(), input.size(0))
        ...
        #print(i, model.compute_average_flops_cost()/ 1e9 / 2)


        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                  'Loss {loss.val:.4f} ({loss.avg:.4f}) c({lossc.avg:.4f}) a({lossa.avg:.4f}) b({lossb.avg:.4f})\t'
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})\t'
                  'Activations: {act.val:.3f} ({act.avg:.3f})'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,
                      loss=losses, lossa=losses_t, lossc=losses_c, lossb=losses_b, top1=top1, top5=top5, act=activations))
            sys.stdout.flush()
...
def validate(val_loader, model, criterion, epoch, target_rates):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, gender) in enumerate(val_loader):
            target = target.cuda(async=True)
            input = input.cuda()
            input_var = torch.autograd.Variable(input)
            target_var = torch.autograd.Variable(target)
            ...
            for k in range(args.batch_size):
                print(target.data.cpu().numpy()[k],end='')
                print(" ",end='')
                for j in range(33):
                    print(activation_rates[j].cpu().numpy()[k][0][0],end='')
                    print(" ",end='')
                print("\n",end='')
            '''
            # classification loss
